...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.get_password(password):
            flash(f"{user.first_name}, You are successfullly logged in!", "success")
...
def register(): 
    ...
    if form.validate_on_submit():
        ...
        user.save()
        flash("Yay! You've successfully registered","success")
        return redirect(url_for('index'))
    return render_template("register.html", title="Register", form=form, register=True)
            # if u dont write                                    ^^^   this token here ull for sure get that undefined error in jinja2
...
def enrollment():
    ...
    if courseID:
        if Enrollment.objects(user_id=user_id, courseID=courseID):
            flash(f"Oops! you are already registered to this course {courseTitle}","danger")
            return redirect(url_for("courses"))
        else:
            Enrollment(user_id=user_id, courseID=courseID).save()
            flash(f"{user_id}, you are enrolled in {courseTitle}", "success")
...
@app.route("/user")
def user():
    # User(user_id=1, first_name="Thor", last_name="Odinson", email="mightythor@uta.com", password="GodOfThunder").save()
    # User(user_id=2, first_name="Jane", last_name="Foster", email="janefoster@uta.com", password="NobelPrizeWinner").save()
